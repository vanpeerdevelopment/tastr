...
import static eu.tastr.infrastructure.exception.TastrEntityBestaatNietException.tastrEntityBestaatNietException;
...
public class RepositoryImpl<T extends AggregateRoot<ID>, ID extends Id> extends SimpleJpaRepository<T, ID> implements Repository<T, ID> {
    ...
    public T findOneExisting(ID id) {
        T result = findOne(id);
        if (result == null) {
            throw tastrEntityBestaatNietException(id);
        }
        return result;
    }
